...
<form id="PurchaseVat" method="POST" enctype="multipart/form-data">
    ...
    <div class="col-md-4">
      <div class="form-group">
        ...
        <select id="VendorSelect" class="form-control">
          ...
          <option value="AddNewVendor">Add new vendor</option>
          @forelse($vendor as $vendors)
          <option id="pan{{ $vendors->id }}" value="{{ $vendors->id }}" data-id="{{ $vendors->pan_number }}">{{ $vendors->vendor_name }}</option>
          @empty
  ...
  <span id="afterVendorSelection" hidden="hidden">
    <div class="row">
      <div class="col-md-4">
           ...
           <div class="form-group">
              ...
              <ul id="listHolder" class="list-group fixed-bottom" style="display:none;">
                @for($i=0; $i < count($data); $i++)
                  ...
                  @php 
                  $temp_cat = $data[$i]->category_name
                  @endphp
                    <ul class="list-group" style="display: none;">
                      @foreach ($data as $row)
                        @if($row->category_name == $temp_cat)
                          @php 
                           $i++
                          @endphp
                          @php
...
<script type="text/javascript" src="{{asset('js/boardAccountingPurchase.js')}}"></script>
