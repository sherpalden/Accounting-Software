...
var dropdownCatData;//account category data for dropdown treee selection
    ...
    $.ajaxSetup({
        ...
        $.ajax({
            ...
            success: function(response) {
                dropdownCatData = response.message;
                // console.log(dropdownCatData);
...
$(document).on('click', '#Account0', function(e) {
    e.preventDefault();
    $('#listItem').append('<ul id="listHolder" class="list-group fixed-bottom" style="display:none;">');
    for(var i = 0; i < dropdownCatData.length; i++){
        $('#listItem').append('<li class="mainCat list-group-item">\
                               <i class="fa fa-plus"></i>');
        $('#cat').val(dropdownCatData[i].category_name);
        var temp_cat = dropdownCatData[i].category_name;
        $('#listItem').append('<ul class="list-group" style="display: none;">');
        for(var j = 0; j < dropdownCatData.length; j++){
            if(dropdownCatData[j].category_name == temp_cat){
                i++;
                temp_cat = dropdownCatData[j].category_name;
                $('#listItem').append('<li class="item list-group-item">');
                $('#ACC').text(dropdownCatData[j].account_name);
                $('#listItem').append('</li>');
            }
        }
        $('#listItem').append('</ul>');
...
$(document).on('keyup', nam, function(e) {
    ...
    if (description != '' && quantity != '' && rate != '' && amount != '') {
        ...
        $('#elements').append('<div class="row staticRow dynamicRow' + counter + '" style="margin-top: 1%;">\
                ...
                <div class="col-sm-3">\
                    <div id="listItem" class="form-group">\
                      <label class="bmd-label-floating">Account Type</label>\
                      <input type="text" id="Account' + counter + '" name="account' + counter + '" class="inputHolder form-control ' + nam + '" readonly>\
                    </div>\
                </div>\
                <div class="col-sm-2">\
                  <div class="form-group">\
...
$(document).on('change', '#VendorSelect', function(e) {
    e.preventDefault();
    var selected = $('#VendorSelect').val();
    if (selected == 'AddNewVendor') {
        // $('.card-body').empty();
        $.ajax({
            url: '/board/accounting/purchase/vendor',
            method: 'GET',
            beforeSend: function() {
                $('#modalLoading').html('<div class="spinner-grow text-muted"></div>\
                    <div class="spinner-grow text-primary"></div>\
            ...
            success: function(response) {
                $('.card-title').append(' (Add New Vendor)')
...
// add new vendor submit form
$(document).on('click', '#submitID', function(e) {
    e.preventDefault();
    var FormData = {
        VendorName: $('#VendorName').val(),
        PanNo: $('#VendorPanNo').val(),
    ...
    $.ajax({
        url: '/board/accounting/add/purchase/vendor',
        method: 'POST',
        data: FormData,
        beforeSend: function() {
            $('#modalLoading').html('<div class="spinner-grow text-muted"></div>\
...
$(document).on('change', '#VendorSelect', function(){
    ...
    if(id != ''){
        $('#afterVendorSelection').removeAttr('hidden');
